...
namespace Combat
{
    public partial class PredictLogicComponent : RenderEntityComponent
    {
        ...
        public void ConfirmCommand(Command cmd)
        {
            switch (cmd.Type)
            {
            case CommandType.EntityMove:
                ConfirmEntityMove(cmd as EntityMoveCommand);
                break;
            default:
                break;
            }
        ...
        public void OnLogicUpdatePosition(Vector3 offset)
        {
            ...
            if (!can_interpolate || m_interpolation_time < 0)
                return;
            float delta_time;
        ...
        void ConfirmEntityMove(EntityMoveCommand cmd)
        {
            switch (cmd.m_move_type)
            {
            case EntityMoveCommand.DirectionType:
